...
for t = 1:times
    ...
    switch lower(FEATURE)    
        ...
        case 'bag of sift'
            ...
            if ~exist(['vocab_size', num2str(vocab_size), '_t', num2str(t), '_libsvm.mat'], 'file')
                fprintf('No existing visual word vocabulary found. Computing one from training images\n')
                vocab = build_vocabulary(train_image_paths, vocab_size);
                save(['vocab_size', num2str(vocab_size), '_t', num2str(t),'_libsvm.mat'], 'vocab');
            end

            % YOU CODE get_bags_of_sifts.m
            if ~exist(['train_image_feats_size', num2str(vocab_size), '_t', num2str(t), '_libsvm.mat'], 'file')
                train_image_feats = get_bags_of_sifts_libsvm(train_image_paths, vocab_size, t);
                save(['train_image_feats_size', num2str(vocab_size), '_t', num2str(t), '_libsvm.mat'], 'train_image_feats');
            else
                load(['train_image_feats_size', num2str(vocab_size), '_t', num2str(t),'_libsvm.mat']);
            end

            if ~exist(['test_image_feats_size', num2str(vocab_size),  '_t', num2str(t),'_libsvm.mat'])
                test_image_feats  = get_bags_of_sifts_libsvm(test_image_paths, vocab_size, t);
                save(['test_image_feats_size', num2str(vocab_size),  '_t', num2str(t),'_libsvm.mat'], 'test_image_feats');
            else
                load(['test_image_feats_size', num2str(vocab_size),  '_t', num2str(t),'_libsvm.mat']);
            end
...
%                        predicted_categories);
	...
	save('all_acc_libsvm.mat', 'all_accuracy');
...
if ~exist(['final_test_image_feats_size', num2str(final_vocab_size),'_libsvm.mat'])
	final_test_image_feats = get_bags_of_sifts_libsvm(final_test_image_paths, final_vocab_size, max_time);
    save(['final_test_image_feats_size', num2str(final_vocab_size),'_libsvm.mat'], 'final_test_image_feats');
else
	load(['final_test_image_feats_size', num2str(final_vocab_size),'_libsvm.mat']);
end

% Train by linear svm
final_predicted_categories = svm_classify(final_train_image_feats, final_train_labels, final_test_image_feats);
save('final_result_libsvm.mat', 'final_test_image_names', 'final_predicted_categories');
